...
dig = "1digit"
name_scenario = "synthetic_data_v2_delta_$(dig)"
country = "USA"
loop = true
verbose = true
plotting  = false
...
deltas = exp.(range(log(1e-3), log(2e-1), length = 5))

# loading coefficients
@load "../../../figures/cleaning_results/2022-08-09_fit_countries_all_without_services_1digit_df_arr_sorted.jld2" df_arr_sorted
res_fitted = df_arr_sorted[(df_arr_sorted.Model .== Ref(L"\mathcal{M}_{\delta}")) .* (df_arr_sorted.group_size_init .==  16) .* (df_arr_sorted.country .==  "USA"), :Result][1]
...
iter = Iterators.product(1:length(group_sizes), 1:length(σ_synthetics), 1:length(deltas)) |> collect # required for atomic access
...
if !loop # for debugging
    ...
    σ_synthetic = 0.3
    delta = 0.2
    raw_data = simulate(synthetic_model, p = [synthetic_model.mp.p[1:2*N]; 0.; 0.; delta])
    if plotting
        fig, ax = subplots()
    ...
    for (i,m) in enumerate(init_models)
        ...
        pars[:model] = m(model);
        rsim = simu(pars)
        push!(df_results,[rsim[1:end-1]...,  delta, σ_synthetic, odedata, group_size_init])
    end
    df_results[!,"delta"] = [getδ(r.Result.m.mp.p, r.Result.m) for r in eachrow(df_results)]
    df_results[!,"loss"] = [r.Result.res.minloss for r in eachrow(df_results)]
    println(df_results[:,["Model","loss","likelihood", "delta"]])
else
    @info "Parameter intialisation"
    for group_size_init in group_sizes, σ_synthetic in σ_synthetics, delta in deltas
        try
            raw_data = simulate(synthetic_model, p = [synthetic_model.mp.p[1:2*N]; 0.; 0.; delta])
            ic_term = 1. / group_size_init
            ...
            for _ in 1:nruns
                ...
                parlog = Dict{Symbol,Any}(); 
                @pack! parlog = country, ranges, odedata, padding_data, optims, ic_term, prior_scaling, continuity_term, group_size_init, delta, σ_synthetic, group_size_init
                for (i,m) in enumerate(init_models)
                    pars = copy(parlog)
    ...
    Threads.@threads for k in 1:length(pars_arr)
        ...
        if !isempty(p)
            try
                @unpack delta, σ_synthetic, odedata, group_size_init = p
                res = simu(p)
                df_results[k,:] = [res[1:end-1]..., delta, σ_synthetic, odedata, group_size_init]
            catch e
