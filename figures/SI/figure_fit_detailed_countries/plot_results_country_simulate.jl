...
Plotting Fig S2 to S5, using initial conditions of the first segments
to simulate the whole time series.
=#
...
countries_to_test = [["ARE", "HRV",],["DEU", "HUN",], ["NZL", "KOR",], ["GBR", "BEL"]]
...
for (sc,mod_to_test) in enumerate(models_to_test)
    ...
    COLOR_PALETTE_1DIG = [_cmap(i-1) for i in 2:2:20];
    ...
    for (c,country) in enumerate(countries)
        ...
        for (mi, mod) in enumerate(model_labs)
            ...
            fitted = simulate(m, u0 = r.Result.res.u0s_trained[1])

            for i in 1:N
                p = findfirst(sitc_labels[i] .== SITC_LABELS_1DIG)
                # @show tsteps[rg]
                # @show r.Result.res.ranges[ri][p,:]
                ax.plot(tsteps, 
                        fitted[i,:], 
                        # label = (i == 1) && (c == 1) && (ri ==1) ? r.Model : nothing,
                        label = (mi==1) && (c == 1) ? sitc_fulllabels[i] : nothing, 
            ...
            ax.text(0.1,0.9,string(L"R^2 = ", @sprintf "%2.3f" R2(odedata, fitted, LogNormal())), transform = ax.transAxes)
    ...
    display(fig)
    fig.savefig("figure_$(figname)_simulate.png", dpi = 300, bbox_inches="tight")
end
